...
class TicTacToeGame:
    ...
    def play_game(self, player1, player2, num_games):
        players = [player1, player2]
        scores = {player1: 0, player2: 0, 'draw': 0}
        
        for game_num in range(1, num_games + 1):  # Jugar partidas según la cantidad especificada
...
def main():
    ...
    while True:
        # Solicitar al usuario cuántas partidas jugar
        try:
            num_games = int(input("Ingrese el número de partidas a jugar (o -1 para salir): "))
            if num_games == -1:
                break
            ...
            scores = game.play_game(player1, player2, num_games)

            # Graficar los resultados
            game.plot_results(scores)
            print(f"Partidas jugadas: {num_games} | Victorias de X: {scores['X']} | Victorias de O: {scores['O']} | Empates: {scores['draw']}")
