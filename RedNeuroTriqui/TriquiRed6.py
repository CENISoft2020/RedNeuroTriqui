...
class TicTacToeGame:
    ...
    def plot_results(self, iterations):
        ...
        plt.subplot(1, 2, 2)
        plt.plot(x_values, [self.scores['X'] / (i + 1) for i in range(iterations)], label='Aprendizaje X', color='blue')
        plt.plot(x_values, [self.scores['O'] / (i + 1) for i in range(iterations)], label='Aprendizaje O', color='orange')
        plt.xlabel('Partidas')
